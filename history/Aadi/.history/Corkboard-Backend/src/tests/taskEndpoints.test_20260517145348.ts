...
beforeEach(async () => {
  try {
    ...
    await userModel.initialize("testDb", true, "users", url);
...
test("PUT /tasks/:id: 401 when not logged in", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
...
test("PUT /tasks/:id: 403 when a non-owner tries to update", async () => {
  const ownerCookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
...
test("PUT /tasks/:id: 400 on a malformed id", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
  const res = await request(app)
    .put("/tasks/not-a-valid-id")
    .set("Cookie", cookie)
    .send(validTask);
  expect(res.status).toBe(400);
});

test("PUT /tasks/:id: 400 on invalid task data", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
...
test("PUT /tasks/:id: 404 when task does not exist", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
...
test("PUT /tasks/:id: 500 when database is closed", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
...
test("DELETE /tasks/:id: 200 when owner deletes their task", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
...
test("DELETE /tasks/:id: 401 when not logged in", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
...
test("DELETE /tasks/:id: 403 when a non-owner tries to delete", async () => {
  const ownerCookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
...
test("DELETE /tasks/:id: 400 on a malformed id", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
...
test("DELETE /tasks/:id: 404 when task does not exist", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
  ...
  const id = created.body._id;
  await request(app).delete(`/tasks/${id}`).set("Cookie", cookie);
  const res = await request(app).delete(`/tasks/${id}`).set("Cookie", cookie);
  expect(res.status).toBe(404);
});

test("DELETE /tasks/:id: 500 when database is closed", async () => {
  const cookie = await LoginAndGetCookie(
    ownerUser.username,
    ownerUser.password,
  );
