...
test("updateTask: updates fields and returns the new document", async () => {
  ...
  expect(updated.pay).toBe(100);
  expect(updated.timeInMins).toBe(90);
});
...
test("updateTask: throws DatabaseError when id does not exist", async () => {
  await expect(
    model.updateTask(new ObjectId(), {
      name: "FixSink",
...
test("deleteTask: removes the task so getAllTasks returns empty", async () => {
  const inserted = await model.addTask(validTask);
  await model.deleteTask(inserted._id);
  const all = await model.getAllTasks();
  expect(all.length).toBe(0);
});

test("deleteTask: throws DatabaseError when id does not exist", async () => {
  await expect(model.deleteTask(new ObjectId())).rejects.toThrow(DatabaseError);
});
