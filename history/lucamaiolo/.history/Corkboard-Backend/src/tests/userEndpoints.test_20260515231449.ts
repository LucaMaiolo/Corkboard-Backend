...
test("returns 401 if not logged in", async () => {
    const res = await request(app).delete(`/users/${validUser.username}`);
    expect(res.status).toBe(401);
});

test("returns 403 if user tries to delete another user", async() =>{
    await request(app).post("/users").send(validUser);
    await request(app).post("/users").send(otherUser);
    const cookie = await LoginAndGetCookie(validUser.username, validUser.password);
    const res = await request(app).delete(`/users/${otherUser.username}`).set("Cookie", cookie);
    expect(res.status).toBe(403);
});
...
test("successfully deletes user", async () => {
    await request(app).post("/users").send(validUser);
    const cookie = await LoginAndGetCookie(validUser.username, validUser.password);
    const res = await request(app).delete(`/users/${validUser.username}`).set("Cookie", cookie);
    expect(res.status).toBe(200);
});